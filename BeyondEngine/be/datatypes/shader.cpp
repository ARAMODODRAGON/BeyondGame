...
namespace be {
	...
	shader::shader(uint32 id) : m_id(id) {}

	shader* shader::load(const std::string& str, loadset set) {
		if (set == IS_FILEPATH)
			return loadfile(str);
	...
	shader* shader::loadfile(const std::string& filepath) {
		...
		return loadsrc(source);
	}

	shader* shader::loadsrc(const std::string& source) {
