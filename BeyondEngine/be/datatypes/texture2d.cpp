...
namespace be {
	...
	texture2d* texture2d::load(const std::string& filepath, const textureopts& options) {
		...
		// create texture node and return
		texture2d* t = new texture2d();
		t->m_id = textureID;
		t->m_size = glm::uvec2(width, height);
		t->m_format = realformat;
		t->m_wrap = options.wrap;
		t->m_filter = options.filter;
		return t;
	}

	texture2d::texture2d(uint32 id, glm::uvec2 size, const textureopts& options)
		: m_id(id), m_size(size), m_format(options.format), m_wrap(options.wrap), m_filter(options.filter) {}

	texture2d::texture2d(texture2d&& other) {
		m_id = other.m_id; other.m_id = 0;
		m_size = other.m_size; other.m_size = glm::uvec2(0);
