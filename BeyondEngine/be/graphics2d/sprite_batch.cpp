...
namespace be {
	namespace graphics2d {
		...
		sprite_batch::sprite_batch(shared<content::shader> s)
			...
			if (s) m_shader = s;
				...
				m_shader.reset(new content::shader(source, content::shader::IS_SOURCE));
			}
		...
		void sprite_batch::push(const std::vector<sprite_vertex>& verticies, content::texture2d* texture) {
		...
		sprite_batch::batch* sprite_batch::get_batch(content::texture2d* texture) {
		...
		uint32 sprite_batch::get_index(batch* b, content::texture2d* texture) {
